...
{
    defineTypeNameAndDebug(mDoFSystemState, 0);
...
Foam::functionObjects::mDoFSystemState::mDoFSystemState
(
    const word& name,
    const Time& runTime,
...
bool Foam::functionObjects::mDoFSystemState::read(const dictionary& dict)
{
...
void Foam::functionObjects::mDoFSystemState::writeFileHeader(const label)
{
...
bool Foam::functionObjects::mDoFSystemState::execute()
{
    return true;
}


const Foam::mDoFSystemMotion&
Foam::functionObjects::mDoFSystemState::motion() const
{
    const dynamicMotionSolverFvMesh& mesh =
        refCast<const dynamicMotionSolverFvMesh>(obr_);

    return (refCast<const mDoFSystemMotion>(mesh.motion()));
}


Foam::vector
Foam::functionObjects::mDoFSystemState::velocity() const
{
    return motion().v();
}
...
Foam::vector
Foam::functionObjects::mDoFSystemState::angularVelocity() const
{
...
bool Foam::functionObjects::mDoFSystemState::write()
{
    logFiles::write();

    if (Pstream::master())
    {
        const mDoFSystemMotion& motion = this->motion();
