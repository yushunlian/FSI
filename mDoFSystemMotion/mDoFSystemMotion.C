...
namespace Foam
{
    defineTypeNameAndDebug(mDoFSystemMotion, 0);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::mDoFSystemMotion::applyRestraints()
{
...
Foam::mDoFSystemMotion::mDoFSystemMotion()
:
    motionState_(),
    motionState0_(),
    restraints_(),
...
:
    ...
    solver_(mDoFSolver::New(dict.subDict("solver"), *this))
{
    addRestraints(dict);
...
Foam::mDoFSystemMotion::mDoFSystemMotion
(
    const mDoFSystemMotion& sDoFRBM
)
:
...
void Foam::mDoFSystemMotion::addRestraints
(
    const dictionary& dict
)
{
...
void Foam::mDoFSystemMotion::addConstraints
(
    const dictionary& dict
)
{
    if (dict.found("constraints"))
    {
        ...
        forAllConstIter(IDLList<entry>, constraintDict, iter)
        {
            if (iter().isDict())
            {
                ...
                (
                    i,
                    mDoFSystemMotionConstraint::New
                    (
                        iter().keyword(),
                        iter().dict(),
                        *this
...
void Foam::mDoFSystemMotion::updateAcceleration
(
...
void Foam::mDoFSystemMotion::update
(
    bool firstIter,
    const vector& fGlobal,
    const vector& tauGlobal,
...
void Foam::mDoFSystemMotion::status() const
{
...
Foam::tmp<Foam::pointField> Foam::mDoFSystemMotion::transform
(
